...
\section{Exploratory Data Analysis (EDA)}
    ...
    \subsection{Statistical Summary \& Visualizations}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.48\textwidth]{dist_pie.png}
            \caption{isFraud Distribution}
            \label{fig:dist-pie}
        \end{figure}

        An analysis of the \texttt{TransactionAmt} feature revealed a heavily right-skewed distribution, typical of financial data where small transactions are frequent and large ones are rare. To address this, we applied a log-transformation ($\log(1+x)$), which normalized the distribution (Figure \ref{fig:dist-comparison}), making it more suitable for linear classifiers like the SVM.

        \begin{figure}[H]
            \centering
            \begin{minipage}{0.48\textwidth}
                \centering
                \includegraphics[width=\linewidth]{distamt_original.png}
                \caption{Original Skewed Distribution}
                \label{fig:distamt-orig}
            \end{minipage}\hfill
            \begin{minipage}{0.48\textwidth}
                \centering
                \includegraphics[width=\linewidth]{distamt_log.png}
                \caption{Log-Transformed Distribution}
                \label{fig:distamt-log}
            \end{minipage}
            \caption{Transaction Amount Distribution. The log-transformation (Right) reveals a underlying normal-like structure, correcting the extreme skew observed in the raw data (Left).}
...
\section{Data Pre-Processing \& Cleaning}
    ...
    \subsection{Encoding Categorical Features \cite{b5}}
        To convert categorical variables (e.g., \texttt{ProductCD}) into a machine-readable numeric format, we applied \textbf{Label Encoding}. 
        \par
...
\section{Model Comparison}
    \subsection{Performance Analysis}
        ...
        \textbf{XGBoost} emerged as the best model, achieving the highest AUC (0.963) and F1-Score (0.70). It prioritized precision (93\%), minimizing false positives to ensure a smooth user experience, while capturing 56\% of fraud cases.
    ...
    \centering
        ...
        \end{minipage}
        \caption{Comparison of Confusion Matrices across the three evaluated models.}
    ...
    \begin{table}[H]
        \centering
        \caption{Metrics Comparison: Training, Validation, and AUC-ROC}
